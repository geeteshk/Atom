...
if (obj_input.dash_key_two) {
    ...
    if (speaker != noone) {
        ...
        if (instance_exists(obj_player)) {
            obj_player_two.persistent = false;
        }
...
player_direction = point_direction(0, 0, obj_input.x_axis_two, obj_input.y_axis_two);

// Get length
if (obj_input.x_axis_two == 0 && obj_input.y_axis_two == 0) {
    length = 0;
    audio_stop_sound(snd_footstep);
} else {
    length = player_speed;
